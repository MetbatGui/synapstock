...
async def test_worker_processes_pending_event_success(mock_outbox):
    ...
    event_item = {
        "id": "12345_uuid",
        "event": {"event_class": "StockAddedToBoard", "data": {"ticker": "005930", "board_id": "theme_IT"}},
        "status": "PENDING",
        "retry_count": 0
    }
    ...
    mock_handler.assert_called_once_with(event_item["event"])
...
async def test_worker_processes_pending_event_failure_and_retries(mock_outbox):
    ...
    event_item = {
        "id": "12345_uuid",
        "event": {"event_class": "StockAddedToBoard", "data": {}},
        "status": "PENDING",
        "retry_count": 0,

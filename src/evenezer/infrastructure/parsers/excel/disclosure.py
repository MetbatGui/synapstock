...
from evenezer.domain.statistics.models import BondWithWarrants, BonusIssue, ConvertibleBond, PaidInCapitalIncrease
        ...
        for sheet_name, df in sheets_dict.items():
            ...
            for _, row in new_df.iterrows():
                try:
                    ...
                    item = self._map_row_to_paid_in_capital_increase(row, name_raw, date_val, is_corr, disc_date)
                    results.append(item)
                except Exception as e:
                    logger.error(f"[DisclosureParser] 유상증자 파싱 실패: {e}")
        return results

    def _map_row_to_paid_in_capital_increase(self, row: pd.Series, name_raw: Any, date_val: str, is_corr: bool, disc_date: str) -> PaidInCapitalIncrease:
        """Pandas Series 로우 데이터를 PaidInCapitalIncrease 도메인 모델로 맵핑합니다."""
        ...
        for sheet_name, df in sheets_dict.items():
            ...
            for _, row in new_df.iterrows():
                try:
                    ...
                    item = self._map_row_to_bonus_issue(row, name_raw, date_val, is_corr, disc_date)
                    results.append(item)
                except Exception as e:
                    logger.error(f"[DisclosureParser] 무상증자 파싱 실패: {e}")
        return results

    def _map_row_to_bonus_issue(self, row: pd.Series, name_raw: Any, date_val: str, is_corr: bool, disc_date: str) -> BonusIssue:
        """Pandas Series 로우 데이터를 BonusIssue 도메인 모델로 맵핑합니다."""
        return BonusIssue(
            date=date_val,
            name=self._clean_stock_name(self.to_str(name_raw)),
            is_correction=is_corr,
        ...
        for sheet_name, df in sheets_dict.items():
            ...
            for _, row in new_df.iterrows():
                try:
                    ...
                    item = self._map_row_to_convertible_bond(row, name_raw, date_val)
                    results.append(item)
                except Exception as e:
                    logger.error(f"[DisclosureParser] 전환사채 파싱 실패: {e}")
        return results

    def _map_row_to_convertible_bond(self, row: pd.Series, name_raw: Any, date_val: str) -> ConvertibleBond:
        """Pandas Series 로우 데이터를 ConvertibleBond 도메인 모델로 맵핑합니다."""
        ...
        for sheet_name, df in sheets_dict.items():
            ...
            for _, row in new_df.iterrows():
                try:
                    ...
                    item = self._map_row_to_bond_with_warrants(row, name_raw, date_val)
                    results.append(item)
                except Exception as e:
                    logger.error(f"[DisclosureParser] BW 파싱 실패: {e}")
        return results

    def _map_row_to_bond_with_warrants(self, row: pd.Series, name_raw: Any, date_val: str) -> BondWithWarrants:
        """Pandas Series 로우 데이터를 BondWithWarrants 도메인 모델로 맵핑합니다."""
        return BondWithWarrants(
            date=date_val,
            name=self._clean_stock_name(self.to_str(name_raw)),
            is_correction="정정" in str(self.get_val(row, "기재정정여부", "정정여부") or ""),
            bond_round=self.to_str(self.get_val(row, "회차", "회 차")),

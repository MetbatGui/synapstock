...
async def get_quarters(
    metric: FinancialMetric = Query(FinancialMetric.OPERATING_PROFIT)
):
...
async def get_top_growers(
    ...
    """성장률 상위 종목을 조회합니다."""
    try:
        results = financial_service.get_top_growers(metric, target_quarter, top_n)
...
async def get_consecutive_growers(
    ...
    try:
        results = financial_service.get_consecutive_growers(metric, target_quarter, count)
        return [
            {
                "stock_name": item.stock_name,
                "current_value": item.current_value,

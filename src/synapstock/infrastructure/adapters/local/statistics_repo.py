...
class LocalStatisticsRepository:
    ...
    def save_daily_ranking(self, ranking: DailyMarketRanking):
        ...
        market_val = ranking.market.value if hasattr(ranking.market, 'value') else ranking.market
        subject_val = ranking.subject.value if hasattr(ranking.subject, 'value') else ranking.subject

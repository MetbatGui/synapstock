...
export const bonusIssueView = {
            ...
            <div class="stats-detail-container animate-fade-in">
                ...
                <div class="stats-info-footer">
                    <div style="margin-right: auto; font-size: 0.85rem; color: #9ca3af;">
                         ...
                         ${item.parent_rcp_no ? `<br/><i class="fas fa-link"></i> 상위공시: <a href="#" onclick="bonusIssueView.jumpToHistory('${item.parent_rcp_no}'); return false;" style="color:var(--accent-blue); text-decoration:underline;">${item.parent_rcp_no}</a>` : ''}
                    </div>
    ...
    jumpToHistory: function (rcpNo) {
        ...
        let targetRow = document.querySelector(`.bi-row[data-rcp-no="${rcpNo}"]`);
        ...
        setTimeout(() => {
            targetRow = document.querySelector(`.bi-row[data-rcp-no="${rcpNo}"]`);
            if (targetRow) {

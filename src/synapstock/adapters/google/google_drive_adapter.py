...
class GoogleDriveAdapter(StoragePort):
    ...
    def sync_pdf_reports(self, local_dir: str, drive_folder_path: str):
        ...
        """
        print(f"[GoogleDrive] PDF 동기화 시작: Local={local_dir} -> Drive={drive_folder_path}")
        ...
        if not os.path.exists(local_dir):
            print(f"[GoogleDrive] 로컬 디렉토리가 존재하지 않습니다: {local_dir}")
            return
            
        local_files = [f for f in os.listdir(local_dir) if f.lower().endswith('.pdf')]
        print(f"[GoogleDrive] 로컬 파일 개수: {len(local_files)}")
        print(f"[GoogleDrive] 드라이브 내 기존 파일 개수: {len(drive_file_names)}")
        ...
        for filename in local_files:
            ...
            with open(local_path, 'rb') as f:
                ...
                if self.put_file(drive_path, data):
                    print(f"[GoogleDrive] 업로드 완료: {filename}")
                    count += 1
                else:
                    print(f"[GoogleDrive] 업로드 실패: {filename}")
                    
        print(f"[GoogleDrive] PDF 동기화 완료: {count}개 파일 업로드됨.")
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(temp_path):
                    os.remove(temp_path)
                print(f"[GoogleDrive] 다운로드 중 오류: {e}")
                return False
        except Exception as e:
            print(f"[GoogleDrive] 파일 조회 중 오류: {e}")
            return False

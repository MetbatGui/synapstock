...
class TestDartDisclosureAdapterCache:
    ...
    async def test_caching_behavior(self, mock_post, mock_html_response):
        ...
        results1 = await adapter.get_recent_disclosures(ticker)
        ...
        results2 = await adapter.get_recent_disclosures(ticker)
    ...
    @patch("evenezer.infrastructure.adapters.disclosure.disclosure_adapter.requests.post")
    async def test_cache_expiration(self, mock_post, mock_html_response):
        ...
        # Act - 1st call (캐시 적재)
        await adapter.get_recent_disclosures(ticker)
        assert mock_post.call_count == 1
    ...
    @patch("evenezer.infrastructure.adapters.disclosure.disclosure_adapter.requests.post")
    async def test_clear_cache(self, mock_post, mock_html_response):
        ...
        # 두 종목에 대해 캐시 적재
        await adapter.get_recent_disclosures(ticker1)
        await adapter.get_recent_disclosures(ticker2)
        assert mock_post.call_count == 2

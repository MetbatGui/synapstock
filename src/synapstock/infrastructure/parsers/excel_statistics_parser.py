...
class ExcelStatisticsParser:
    ...
    def parse_paid_in_capital_increase(content: bytes) -> list[PaidInCapitalIncrease]:
        ...
        엑셀 내의 모든 시트(연도별 등)를 순회하며 파싱합니다.
        맨 윗줄이 비어있거나 헤더 위치가 다른 경우를 자동으로 감지합니다.
        ...
        """
        # 엑셀 헤더가 1행이 아닐 수 있으므로 header=None으로 먼저 읽음
    ...
    @staticmethod
    def parse_bond_with_warrants(content: bytes) -> list[BondWithWarrants]:
        """신주인수권부사채(BW) 발행 결정 엑셀 파일을 파싱하여 도메인 모델 리스트로 변환합니다.
            ...
            for _, row in new_df.iterrows():
                try:
                    ...
                    item = BondWithWarrants(
                        ...
                        maturity_date=to_str(get_val(row, "사채의만기일", "만기일")),
                        issue_method=to_str(get_val(row, "사채발행방법", "발행방법")),
                        warrant_ratio=to_float(get_val(row, "신주인수권비율", "비율")),
                        exercise_price=to_int(get_val(row, "행사가액", "가격")),
                        new_shares=to_int(get_val(row, "행사에따라발행할주식수", "인수주식수")),
                        shares_ratio=to_float(get_val(row, "주식총수대비비율", "총수대비비율")),
                        exercise_start_date=to_str(get_val(row, "권리행사기간시작일", "행사시작일")),
                        exercise_end_date=to_str(get_val(row, "권리행사기간종료일", "행사종료일")),
                        subscription_date=to_str(get_val(row, "청약일")),
                        payment_date=to_str(get_val(row, "납입일")),
                        board_resolution_date=to_str(get_val(row, "이사회결의일", "결의일")),
                        rcp_no=to_str(get_val(row, "접수번호")),
                    ...
                    logger.error(f"[ExcelParser] BW 파싱 실패 (시트: {sheet_name}): {e}")
                    continue

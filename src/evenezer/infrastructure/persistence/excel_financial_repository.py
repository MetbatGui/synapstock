...
class ExcelFinancialRepository(FinancialRepository):
    ...
    def __init__(self, file_path: str):
        ...
        # 인메모리 메모이제이션 캐시용 변수
        self._cache = {}  # FinancialMetric -> list[FinancialStatement]
        self._quarters_cache = {}  # FinancialMetric -> list[str]
        self._last_modified_at = 0.0
    ...
    def get_all_quarters(self, metric: FinancialMetric) -> list[str]:
        ...
        current_mtime = self.get_last_modified_time()
        if current_mtime != self._last_modified_at or metric not in self._quarters_cache:
            try:
                df = pd.read_excel(self.file_path, sheet_name=sheet_idx, nrows=0)
                if len(df.columns) > 1:
                    # 첫 번째 컬럼(종목명) 제외한 나머지 분기 목록을 역순(최신순)으로 반환
                    self._quarters_cache[metric] = [str(c) for c in reversed(df.columns[1:])]
                else:
                    self._quarters_cache[metric] = []
            except Exception:
                self._quarters_cache[metric] = []
            self._last_modified_at = current_mtime

        return self._quarters_cache[metric]
    ...
    def load_all(self, metric: FinancialMetric) -> list[FinancialStatement]:
        ...
        current_mtime = self.get_last_modified_time()
        if current_mtime != self._last_modified_at or metric not in self._cache:
            try:
                # 엑셀 파일 로드 (지정된 시트)
